...
public class migrar {
	...
	/**
	 * La clase migrar trabaja con una ruta estática que debemos hardcodear. 
	 * En dicha ruta depositaremos el fichero a migrar.
	 * 
	 ...
	 * nuestras columnas. 
	 * 
	 * static final String PATH: INDICAR AQUÍ RUTA ABSOLUTA DONDE ESTÁ EL TXT QUE QUIERES IMPORTAR 
	 * static final String FICHERO: Nombre del fichero a importar, siempre en la ruta PATH.
	 * static final String FICHERO_RES: Nombre del fichero de resultado.
	 * static final String FICHERO_SQL: Nombre del fichero de insert que no han podido hacerse. 
	 * (No tiene porque generarse)
	 */
	...
	public static void main(String[] args) {
		...
		int contador_no = 0;//Cuenta registros que no se persisten por no venir en el formato debido 
							//(7 campos separados por coma)
		int contador_fallos = 0;//Cuenta las INSERT incompatibles con BBDD por longitud
		int contador_columnas = 0;//Cuenta las columnas incompatibles con BBDD por longitud
		...
		try {
			...
			prepararArchivo(); //creamos el fichero de SQLs incompatibles en blanco
			...
			while(  (linea=br.readLine()) !=null)  {
				...
				if (aCampos.length == 7) {
					autorizado = true;
					//No nos responsabilizamos de datos corruptos ni incompletos
					//ni nada, todo lo que tenga formato de 7 campos, lo cogemos
	 ...
	 *  @param int contador_si, contador de registros importados correctamente
	 *  @param int contador_no, contador de registros que no se importan por no tner 7 campos
	 *  @param int contador_fallos, contador de registros que no se importan por ser demasiado grandes para nuestras columnas
	 *  @param int contador_columnas, desglose del contador anterior, por cada columna. Un registro solo suma 1
	 *   	   a contador_fallos, pero puede tener entre 1 y N (campos) columnas comprometidas
	 *   
	 *  Lo que hace esta función es escribir un txt con estos datos, siendo los dos últimos
	 *  opcionales, por lo que solo los mostrará si nos hemos encontrado con sqls que pretendían 
	...
	private static void fichero_resumen(int contador_si, int contador_no, int contador_fallos, int contador_columnas) {
		...
		try{		
			...
			if (contador_fallos > 0) {
				pw.println(" ");
				pw.println("Además, "+contador_fallos+" INSERTs no eran compatibles con el tamaño de sus columnas, por "+contador_columnas+" campos");
	...
	//Simplemente borramos el fichero de arcivo de sql por si existe de ejecuciones anteriores
	//Lo hacemos fuera del bucle ya que si no, estariamos borrandolo con cada sql incompatible 
